...
async function sendBillViaEmailJS(toEmail, toName, orderObj) {
    ...
    const deliveryAddr = orderObj.delivery ? orderObj.delivery.address : 'Address provided at checkout';
    const formattedTotal = `₹${parseFloat(orderObj.total || 0).toFixed(2)}`;
    const formattedNumTotal = `${parseFloat(orderObj.total || 0).toFixed(2)}`;
...
window.updateBookingStatus = async function(orderId, newStatus) {
    ...
    if (order) {
        ...
        if (newStatus === 'Delivered' && order.delivery && order.delivery.email) {
            sendBillViaEmailJS(order.delivery.email, order.delivery.name, order).then(res => {
                if (res && res.success) {
                    showToast(`📩 Delivery Invoice Bill sent to <strong>${order.delivery.email}</strong> via EmailJS!`, 'success');
                }
            });
        }
